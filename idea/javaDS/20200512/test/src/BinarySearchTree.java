public class BinarySearchTree {
    ...
    public Node insert (int key,int value) {
        ...
        while(cur != null) {
            if(key < cur.key) {
                ...
                //针对key重复的情况，处理方法有很多种
                //a).让插入操作直接失败
                //b).不创建新节点，把当前节点的value给改成新的value【此处采取该方法】
                cur.value = value;
                return cur;
            }
        }
    ...
    public void remove (int key) {
        //1.先找到要删除的节点的位置，找到的同时记录该节点的父节点位置
        Node cur = root;
        Node parent = null;
        while(cur != null) {
            if(key < cur.key) {
    ...
    private void removeNode(Node cur, Node parent) {
        ...
        if(cur.left == null) {
            ...
            if (cur == root) {
                ...
                //1.2 cur不为根节点，且为父节点的左子树
                parent.left = cur.right;
            }else if (cur == parent.right){
                //1.3  cur不为根节点，且为父节点的右子树
            ...
            if(cur == root) {
                ...
                //2.2  cur不为根节点，且为父节点的左子树
                parent.left = cur.left;
            }else if(cur == parent.right) {
                //2.3  cur不为根节点，且为父节点的右子树
                parent.right = cur.left;
            }
        }else {
            //3，左右子树都有的情况
            //a) 先找到替罪羊节点，同时记录下替罪羊的父节点
            ...
            //循环结束之后，scapegoat就指向了右子树中的最左侧节点，即要删除节点右子树中的最小节点
            // 也可以查找要删除节点左子树的最大节点，
