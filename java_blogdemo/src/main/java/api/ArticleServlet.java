...
public class ArticleServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
       resp.setContentType("text/html; charset=utf-8");
        //1.验证用户是否已经登陆了，如果尚未登陆，就提示用户先登陆
        HttpSession httpSession = req.getSession(false);
        if(httpSession == null) {
            //当前是未登录状态
            String html = HtmlGenerator.getMessagePage("请先登陆", "login.html");
    ...
    private void getOneArticle(int articleId, User user, HttpServletResponse resp) throws IOException {
        ...
        //为了验证换行消失的问题，在这个环节把Article中的正文打印出来，看看是不是带换行
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        resp.setContentType("text/html; charset=utf-8");
        //1,判定用户的登陆状态,如果用户尚未登陆，提醒用户进行登陆
        HttpSession httpSession = req.getSession(false);
        if(httpSession == null){
            String html = HtmlGenerator.getMessagePage("您尚未登陆，请登录！","login.html");
            resp.getWriter().write(html);
            return;
        }
        User user = (User) httpSession.getAttribute("user");
        //2,从请求中读取到浏览器提交的数据（title，content），并进行简单校验
